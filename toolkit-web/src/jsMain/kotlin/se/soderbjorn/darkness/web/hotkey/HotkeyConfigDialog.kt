 ...
 * [se.soderbjorn.darkness.web.toCssVarMap] for the canonical token list).
 * The footer buttons reuse the shared `.dt-modal-btn*` classes from
 * `darkness-toolkit.css` (the same ones [se.soderbjorn.darkness.web.showConfirmDialog]
 * uses) so the primary/cancel buttons follow the active theme exactly like
 * every other toolkit dialog; hosts must have called
 * `injectDarknessToolkitStyles` at boot (they already must for ConfirmDialog).
...
private object HotkeyConfigDialogHost {
    ...
    // darkness-toolkit.css. Fallback values after each token mirror the
